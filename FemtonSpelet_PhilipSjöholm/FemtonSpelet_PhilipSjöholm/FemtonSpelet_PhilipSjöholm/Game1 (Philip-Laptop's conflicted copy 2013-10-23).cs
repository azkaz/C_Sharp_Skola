...
namespace FemtonSpelet_PhilipSjöholm
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (lockGame == false)
            {
                if (_mouseState.LeftButton == ButtonState.Pressed && _oldMouseState.LeftButton == ButtonState.Released)
                {
                    ...
                    else if (Math.Abs(indexX - whiteTileIndex_X) == 2 && Math.Abs(indexY - whiteTileIndex_Y) == 0 || Math.Abs(indexX - whiteTileIndex_X) == 0 && Math.Abs(indexY - whiteTileIndex_Y) == 2)
                    {
                        for (int i = 0; i < 3; i++)
                        {
                            if (indexX >= 0 || indexX <= 3 || indexY >= 0 || indexY <= 3 )
                            {
                               SwapOut(whiteTileIndex_X  , whiteTileIndex_Y, indexX, indexY - 1);
                               SwapOut(whiteTileIndex_X, whiteTileIndex_Y, indexX, indexY);
                               CheckTiles();  
                            } 
                        }
                    ...
                    else if (Math.Abs(indexX - whiteTileIndex_X) == 3 && Math.Abs(indexY - whiteTileIndex_Y) == 0 || Math.Abs(indexX - whiteTileIndex_X) == 0 && Math.Abs(indexY - whiteTileIndex_Y) == 3)
                    {
                        SwapOut(whiteTileIndex_X, whiteTileIndex_Y, indexX, indexY);
                    }*/
                }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            base.Draw(gameTime);
            spriteBatch.Begin();
            spriteBatch.DrawString(_spriteFont, "TILES CLICKED: ", new Vector2(0, 0), Color.White);
            spriteBatch.End();
            for (int i = 0; i < tileArray.GetLength(0); i++)
            {
                for (int j = 0; j < tileArray.GetLength(1); j++)
